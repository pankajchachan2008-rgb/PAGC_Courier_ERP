...
def auto_heal_db():
    try:
        ...
        tables = [
            ...
            "CREATE TABLE IF NOT EXISTS outward_register (id INT AUTO_INCREMENT PRIMARY KEY, entry_date DATE, awb_no VARCHAR(100), origin_station VARCHAR(100), out_station VARCHAR(100), destination VARCHAR(100), weight VARCHAR(50), info TEXT, outward_no VARCHAR(100), manifest_no VARCHAR(100), finalized INT DEFAULT 0)",
            ...
            "CREATE TABLE IF NOT EXISTS delivery_register (id INT AUTO_INCREMENT PRIMARY KEY, entry_date DATE, delivery_boy VARCHAR(100), delivery_area VARCHAR(100), awb_no VARCHAR(100), receiver_name VARCHAR(100), info TEXT, drs_no VARCHAR(100), finalized INT DEFAULT 0)",
...
def dashboard():
    ...
    q_l = "SELECT awb_no, booking_date, dest_name, status, total_amount FROM shipments WHERE 1=1"
    if session.get('role') != 'ADMIN':
...
def rates():
    ...
    conn.close()
    html = """<div class="card"><h3 style="margin-top:0; color:#0E8A6D;"><i class="fas fa-tags"></i> Add Rate</h3><form method="POST" class="grid-4" style="align-items:end;"><div style="grid-column: span 2;"><label>Customer</label><select name="cust_id"><option value="">-- Generic --</option>{% for c in custs %}<option value="{{ c.id }}">{{ c.name }}</option>{% endfor %}</select></div><div><label>Origin State</label><input name="ostate" required></div><div><label>Dest State</label><input name="dstate" required></div><div><label>Min Wt</label><input type="number" step="0.1" name="min_w" value="0.0"></div><div><label>Max Wt</label><input type="number" step="0.1" name="max_w" value="999.0"></div><div><label>Fixed (₹)</label><input type="number" step="0.1" name="fixed" value="0.0"></div><div><label>Per KG (₹)</label><input type="number" step="0.1" name="per_kg" value="0.0"></div><div><label>GST %</label><input type="number" step="0.1" name="gst" value="18.0"></div><div><button type="submit" class="btn btn-blue" style="width:100%;"><i class="fas fa-save"></i> Save Rate</button></div></form></div><div class="card"><h3>Active Rate Cards</h3><table><tr><th>Customer</th><th>Route</th><th>Wt Slab</th><th>Fixed</th><th>Per KG</th><th>GST</th><th>Del</th></tr>{% for r in r_list %}<tr><td><strong>{{ r.name or 'Generic' }}</strong></td><td>{{ r.origin_state_code }} &rarr; {{ r.dest_state_code }}</td><td>{{ r.min_weight }} - {{ r.max_weight }} KG</td><td style="color:#0E8A6D; font-weight:bold;">₹{{ r.fixed_charge }}</td><td style="color:#C9A24B; font-weight:bold;">₹{{ r.per_kg_rate }}</td><td>{{ r.gst_rate }}%</td><td><a href="/rates?delete={{ r.id }}" class="btn btn-red" style="padding:4px 8px;"><i class="fas fa-trash"></i></a></td></tr>{% endfor %}</table></div>"""
...
def booking():
    ...
    with conn.cursor() as c:
        ...
        c.execute("SELECT s.id, s.awb_no, c.name as customer_name, s.dest_station, s.dest_name, s.dest_state_code, s.weight_kg, (s.cgst+s.sgst+s.igst) as gst, s.total_amount, s.status, s.booking_date FROM shipments s LEFT JOIN customers c ON c.id=s.customer_id ORDER BY s.id DESC LIMIT 50")
        recent = c.fetchall()
    conn.close()
    ...
    <div class="card" style="max-width:950px; margin:auto; margin-top:20px;">
        ...
        {% for r in recent %}<tr><td>{{ r.id }}</td><td style="color:#0E8A6D; font-weight:bold;">{{ r.awb_no }}</td><td>{{ r.customer_name or '' }}</td><td>{{ r.dest_station }}</td><td>{{ r.dest_name }} ({{ r.dest_state_code }})</td><td>{{ r.weight_kg }}</td><td>₹{{ r.gst or 0 }}</td><td style="font-weight:bold;">₹{{ r.total_amount }}</td><td><span class="badge">{{ r.status }}</span></td><td>{{ r.booking_date }}</td></tr>{% endfor %}
        </table>
    </div>
...
def shipments():
    ...
    with conn.cursor() as c:
        q = "SELECT s.id, s.awb_no, COALESCE(c.name,'') as cname, s.dest_name, s.status, s.current_location, s.weight_kg, s.total_amount, s.booking_date FROM shipments s LEFT JOIN customers c ON s.customer_id = c.id WHERE 1=1"
        params = []
    ...
    <div class="card"><table style="font-size:13px;"><tr><th>Id</th><th>Awb</th><th>Customer</th><th>Destination</th><th>Status</th><th>Location</th><th>Weight</th><th>Total</th><th>Date</th><th>Actions</th></tr>
        {% for r in rows %}<tr>
            <td>{{ r.id }}</td><td style="color:#0B1F3A; font-weight:800;">{{ r.awb_no }}</td><td>{{ r.cname }}</td><td>{{ r.dest_name }}</td>
            <td><span class="badge b-del">{{ r.status }}</span></td><td>{{ r.current_location }}</td><td>{{ r.weight_kg }}</td><td style="color:#0E8A6D; font-weight:bold;">₹{{ r.total_amount or 0 }}</td><td>{{ r.booking_date }}</td>
            <td>
                <a href="/edit_shipment/{{ r.id }}" class="btn btn-blue" style="padding:4px 8px; font-size:11px; border-radius:4px;"><i class="fas fa-edit"></i> Edit</a>
                <a href="/print/label/{{ r.awb_no }}" target="_blank" class="btn btn-ghost" style="padding:4px 8px; font-size:11px; border-radius:4px;"><i class="fas fa-print"></i> Lbl</a>
                <a href="/print/receipt/{{ r.awb_no }}" target="_blank" class="btn btn-gold" style="padding:4px 8px; font-size:11px; border-radius:4px;"><i class="fas fa-file-invoice-dollar"></i> Rec</a>
                <a href="/shipments?delete={{ r.id }}" onclick="return confirm('Delete this shipment?');" class="btn btn-red" style="padding:4px 8px; font-size:11px; border-radius:4px;"><i class="fas fa-trash"></i></a>
            </td>
        </tr>{% endfor %}</table></div>
    """
...
# ==========================================
# 📤 8. OUTWARD HUB & EXCEL IMPORT (EXACT MATCH)
# ==========================================
...
def outward():
                    ...
                    if not b_items: flash(f"Bag {awb} is empty.", "error")
                        ...
                        for bi in b_items:
                            ...
                            if not c.execute("SELECT id FROM outward_register WHERE awb_no=%s AND finalized=0", (sub_awb,)):
                                c.execute("INSERT INTO outward_register(entry_date, awb_no, origin_station, out_station, destination, weight, info, finalized) VALUES(%s, %s, %s, %s, %s, %s, %s, 0)", (o_date, sub_awb, session.get('branch','HQ'), o_station, s_dst, s_wt, f"Unpacked {awb}"))
    ...
    with conn.cursor() as c:
        ...
        c.execute("SELECT outward_no, MIN(entry_date) as d, MIN(out_station) as st, COUNT(*) as c, MIN(manifest_no) as m FROM outward_register WHERE finalized=1 GROUP BY outward_no ORDER BY d DESC LIMIT 10"); mans = c.fetchall()
    conn.close()
    ...
    <div class="card" id="content-new" style="border-top-color:#0B1F3A;">
        ...
        <div style="height:300px; overflow-y:auto; border:1px solid #E2E8F0; border-radius:8px;"><table style="margin:0;">
            <tr style="position:sticky; top:0; background:#F8FAFC; z-index:1;"><th>ID</th><th>AWB</th><th>Dest</th><th>Weight</th><th>Info</th><th>Del</th></tr>
            {% for p in pending_list %}<tr><td>{{ p.id }}</td><td style="color:#0B1F3A; font-weight:800; font-size:14px;">{{ p.awb_no }}</td><td>{{ p.destination or '-' }}</td><td style="font-weight:bold;">{{ p.weight or '0' }} kg</td><td>{{ p.info or '-' }}</td><td><a href="/outward?delete={{ p.id }}&date={{ f_date }}&station={{ f_station }}" class="btn btn-red" style="padding:4px 8px; border-radius:4px;"><i class="fas fa-trash"></i></a></td></tr>{% else %}<tr><td colspan="6" style="text-align:center; padding:40px; color:#94A3B8; font-size:15px;">Box is empty. Scan an AWB to start.</td></tr>{% endfor %}
        </table></div>
    ...
    <div class="card" id="content-history" style="display:none; border-top-color:#C9A24B;">
        ...
        <table style="width:100%;"><tr><th>Outward No</th><th>Date</th><th>Station</th><th>Docs</th><th>Manifest No</th></tr>
        {% for m in mans %}<tr><td><strong>{{ m.outward_no }}</strong></td><td>{{ m.d }}</td><td>{{ m.st }}</td><td>{{ m.c }}</td><td>{{ m.m or '-' }}</td></tr>{% endfor %}</table>
    </div>
...
def drs():
    ...
    <div class="grid-2">
        <div class="card" style="border-top-color: #0B1F3A;">
            ...
            <div style="max-height:200px; overflow-y:auto;"><table><tr><th>ID</th><th>Date</th><th>Rider</th><th>Area</th><th>AWB</th><th>Receiver</th></tr>{% for p in pending %}<tr><td>{{ p.id }}</td><td>{{ p.entry_date }}</td><td>{{ p.delivery_boy }}</td><td>{{ p.delivery_area }}</td><td style="font-weight:bold; color:#0E8A6D;">{{ p.awb_no }}</td><td>{{ p.receiver_name }}</td></tr>{% endfor %}</table></div>
        ...
        <div>
            <div class="card" style="border-top-color: #C9A24B;">
                ...
                <div style="max-height:200px; overflow-y:auto;"><table><tr><th>DRS No</th><th>Date</th><th>Rider</th><th>Area</th><th>Total Docs</th></tr>{% for s in sess %}<tr><td><strong>{{ s.drs_no }}</strong></td><td>{{ s.d }}</td><td>{{ s.b }}</td><td>{{ s.a }}</td><td>{{ s.c }}</td></tr>{% endfor %}</table></div>
                <h4 style="margin-top:20px;">DRS Printing</h4>
                <table><tr><th>ID</th><th>DRS No</th><th>Date</th><th>Rider</th><th>Status</th><th>Action</th></tr>{% for d in drs_tbl %}<tr><td>{{ d.id }}</td><td><strong>{{ d.drs_no }}</strong></td><td>{{ d.drs_date }}</td><td>{{ d.rider_name }}</td><td><span class="badge">{{ d.status }}</span></td><td><a href="/print/drs/{{ d.id }}" target="_blank" class="btn btn-blue" style="padding:2px 6px; font-size:11px;">🖨️</a></td></tr>{% endfor %}</table>
            </div>
...
def print_manifest_pdf(mid):
    ...
    c.execute("SELECT s.awb_no, s.dest_station, o.weight, o.info FROM manifest_items mi JOIN shipments s ON s.id=mi.shipment_id JOIN outward_register o ON o.awb_no=s.awb_no WHERE mi.manifest_id=%s", (mid,)); items = c.fetchall()
    c.close(); conn.close()
