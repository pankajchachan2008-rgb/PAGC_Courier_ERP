...
def login():
    if request.method == 'POST':
        ...
        # FIX: Handled missing dict keys safely to prevent 500 error
...
def track():
    ...
    <div class="track-container"><div class="track-card">
        <h3 class="track-title">C.Note / Shipment Tracking Detail</h3>
        <form method="GET" class="search-area"><input type="text" name="awb" value="{{ awb }}" placeholder="Enter C.Note Number" required autofocus><button type="submit">TRACK</button></form>
        {% if error_msg %}<div style="color:red; font-weight:bold; border:1px solid red; padding:5px; background:#FFCCCC;">System Error: {{ error_msg }}</div>
        {% elif awb and not shipment %}<div style="color:red; font-weight:bold; border:1px solid red; padding:5px; background:#FFCCCC;">No record found for C.Note / Document No: {{ awb }}</div>
        {% elif shipment %}
            <div class="info-grid"><div class="info-item"><strong>C.Note No:</strong> <span style="color:red; font-weight:bold; font-size:12px;">{{ shipment.awb_no }}</span><br><strong>Booking Date:</strong> {{ shipment.booking_date }}<br><strong>Origin:</strong> {{ shipment.origin_name or '-' }}<br><strong>Destination:</strong> {{ shipment.dest_name or '-' }} ({{ shipment.dest_station or '-' }})</div><div class="info-item"><strong>Current Status:</strong> <span class="status-hl">{{ shipment.status }}</span><br><strong>Location:</strong> {{ shipment.current_location or '-' }}<br><strong>Weight:</strong> {{ shipment.weight_kg }} KG<br><strong>Pcs:</strong> {{ shipment.quantity or '1' }}</div></div>
            <h3 class="track-title">Tracking History (Movement Detail)</h3>
            {% if events %}<table class="track-table"><thead><tr><th>Date / Time</th><th>Status / Scan Type</th><th>Location & Remarks</th></tr></thead><tbody>{% for e in events %}<tr><td>{{ e.created_at }}</td><td style="font-weight:bold; color:#116B7A;">{{ e.scan_type }}</td><td><strong>{{ e.location or '-' }}</strong> - {{ e.remarks or '' }}</td></tr>{% endfor %}</tbody></table>{% else %}<p style="font-weight:bold; color:red;">No movement history available yet.</p>{% endif %}
        {% endif %}
    </div></div>
    """
    try: from flask import render_template_string; return render_page(f"Tracking {awb}", render_template_string(html, awb=awb, shipment=shipment, events=events, error_msg=error_msg))
    except Exception: return render_template_string("<html><body>" + html + "</body></html>", awb=awb, shipment=shipment, events=events, error_msg=error_msg)
...
def customers():
    ...
    <div style="background: white; border: 1px solid #116B7A; border-top: 3px solid #116B7A;">
        ...
        <div class="agcs-top-bar"><button type="button" class="agcs-btn-grey" onclick="document.getElementById('masterForm').submit()">SAVE</button><button type="button" class="agcs-btn-grey" onclick="document.getElementById('masterForm').reset()">RESET</button><button type="button" class="agcs-btn-grey">DELETE</button><button type="button" class="agcs-btn-grey" onclick="window.location.href='/'">EXIT</button><div style="margin-left: auto; color: #D67A00; font-weight: bold; font-size:14px; padding-right:10px;">Center : {{ session.branch | default('NOHAR') }}/{{ session.branch | default('NOHAR') }}-PANKAJ AGENCY</div></div>
            ...
            <div class="agcs-right-form">
                <form method="POST" id="masterForm" style="margin:0;">
                    ...
                    <table class="agcs-form-table">
                        ...
                        <tr><td class="agcs-label">Address</td><td colspan="3"><input type="text" name="address" id="f_address" class="agcs-input" style="width: 60%; margin-bottom: 2px;"><br><input type="text" class="agcs-input" style="width: 60%; margin-bottom: 2px;"><br><input type="text" class="agcs-input" style="width: 60%;"></td></tr>
...
def delivery_boy():
    ...
    <div style="background: white; border: 1px solid #116B7A; border-top: 3px solid #116B7A;"><h2 class="page-title-green">DELIVERY BOY MASTER ENTRY</h2><div class="agcs-top-bar"><button type="button" class="agcs-btn-grey" onclick="document.getElementById('masterForm').submit()">SAVE</button><button type="button" class="agcs-btn-grey" onclick="document.getElementById('masterForm').reset()">RESET</button><button type="button" class="agcs-btn-grey">DELETE</button><button type="button" class="agcs-btn-grey" onclick="window.location.href='/'">EXIT</button><div style="margin-left: auto; color: #D67A00; font-weight: bold; font-size:14px; padding-right:10px;">Center : {{ session.branch | default('NOHAR') }}</div></div><div class="agcs-container"><div class="agcs-left-list"><div class="list-header" style="color:black; font-style:normal;">CURRENT DELIVERY BOY</div><div style="background: #99CCCC;">{% for b in boys %}<div class="list-item" onclick="loadBoy('{{ b.username }}', '{{ b.full_name }}')">{{ b.full_name | upper }}</div>{% endfor %}</div></div><div class="agcs-right-form"><form method="POST" id="masterForm" style="margin:0;"><div class="list-header" style="text-align:center; background:#DCEBEB; color:black; font-style:normal;">DELIVERY BOY'S DETAILS</div><table class="agcs-form-table" style="width: 80%; margin: 10px auto;"><tr><td class="agcs-label" style="width:20%;">Boy Code</td><td colspan="3"><input type="text" name="code" id="b_code" class="agcs-input" style="width: 30%;" required></td></tr><tr><td class="agcs-label">Full Name</td><td colspan="3"><input type="text" name="name" id="b_name" class="agcs-input" style="width: 80%;" required></td></tr><tr><td class="agcs-label">Address</td><td colspan="3"><input type="text" name="address" class="agcs-input" style="width: 100%; margin-bottom: 2px;"><br><input type="text" class="agcs-input" style="width: 100%; margin-bottom: 2px;"><br><input type="text" class="agcs-input" style="width: 100%;"></td></tr><tr><td class="agcs-label">Area</td><td><input type="text" name="area" class="agcs-input" style="width: 90%;"></td><td class="agcs-label" style="text-align:right;">City</td><td><input type="text" class="agcs-input" style="width: 90%;"></td></tr><tr><td class="agcs-label">State</td><td><input type="text" class="agcs-input" style="width: 90%;"></td><td class="agcs-label" style="text-align:right;">PinCode</td><td><input type="text" class="agcs-input" style="width: 90%;"></td></tr><tr><td class="agcs-label">Phone No.</td><td><input type="text" name="phone" class="agcs-input" style="width: 90%;"></td><td class="agcs-label" style="text-align:right;">Mobile No</td><td><input type="text" class="agcs-input" style="width: 90%;"></td></tr><tr><td class="agcs-label">Ref Prsn</td><td><input type="text" class="agcs-input" style="width: 90%;"></td><td class="agcs-label" style="text-align:right;">Ref. No</td><td><input type="text" class="agcs-input" style="width: 90%;"></td></tr></table></form></div></div></div><script>function loadBoy(code, name) { document.getElementById('b_code').value = code; document.getElementById('b_name').value = name;}</script>
    """
