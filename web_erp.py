...
# ==========================================
# 💎 LUXURY EXECUTIVE THEME (GLASSMORPHISM)
# ==========================================
AGCS_BASE_HTML = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>{{ title }} | AGC Luxury ERP</title>
    ...
    <style>
        ...
        body {
            ...
            background-image: 
                ...
                radial-gradient(at 0% 100%, rgba(226,232,240,0.6) 0px, transparent 50%);
            background-attachment: fixed;
            color: #1e293b;
        }

        /* 🪟 High-End Pearl Glass Cards */
        .card {
        ...
        /* ✨ Luxury Buttons (Sapphire, Ruby, Emerald, Gold) */
        .btn-primary { background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); color: #fff; padding: 10px 20px; border-radius: 10px; font-weight: 600; border: 1px solid rgba(255,255,255,0.2); cursor: pointer; display: inline-block; font-size: 13px; box-shadow: 0 4px 12px rgba(59,130,246,0.3); transition: 0.3s; }
        .btn-primary:hover { box-shadow: 0 6px 16px rgba(59,130,246,0.5); transform: translateY(-1px); }
        
        .btn-danger { background: linear-gradient(135deg, #b91c1c 0%, #ef4444 100%); color: #fff; padding: 10px 20px; border-radius: 10px; font-weight: 600; border: 1px solid rgba(255,255,255,0.2); cursor: pointer; font-size: 13px; box-shadow: 0 4px 12px rgba(239,68,68,0.3); transition: 0.3s; }
        .btn-danger:hover { box-shadow: 0 6px 16px rgba(239,68,68,0.5); transform: translateY(-1px); }

        .btn-success { background: linear-gradient(135deg, #047857 0%, #10b981 100%); color: #fff; padding: 10px 20px; border-radius: 10px; font-weight: 600; border: 1px solid rgba(255,255,255,0.2); cursor: pointer; font-size: 13px; box-shadow: 0 4px 12px rgba(16,185,129,0.3); transition: 0.3s; }
        .btn-success:hover { box-shadow: 0 6px 16px rgba(16,185,129,0.5); transform: translateY(-1px); }

        .btn-warning { background: linear-gradient(135deg, #d97706 0%, #f59e0b 100%); color: #fff; padding: 10px 20px; border-radius: 10px; font-weight: 600; border: 1px solid rgba(255,255,255,0.2); cursor: pointer; font-size: 13px; box-shadow: 0 4px 12px rgba(245,158,11,0.3); transition: 0.3s; }
        .btn-warning:hover { box-shadow: 0 6px 16px rgba(245,158,11,0.5); transform: translateY(-1px); }

        /* 🌫️ Neumorphic Inputs */
        .input-modern {
            width: 100%; padding: 12px 16px; border: 1px solid rgba(203, 213, 225, 0.6);
            border-radius: 12px; font-size: 14px; background: rgba(255, 255, 255, 0.6);
            backdrop-filter: blur(4px); transition: all 0.3s ease; color: #0f172a; font-weight: 600;
        }
        .input-modern:focus {
            background: #ffffff; border-color: #3b82f6; box-shadow: 0 0 0 4px rgba(59,130,246,0.1); outline: none;
        }
        .label-modern { font-size: 11px; font-weight: 700; color: #64748b; margin-bottom: 6px; display: block; text-transform: uppercase; letter-spacing: 0.8px; }
        
        /* 📱 Clean DataTables */
        .table-responsive-wrapper { width: 100%; overflow-x: auto; border-radius: 12px; }
        .datatable { width: 100%; border-collapse: separate; border-spacing: 0; font-size: 13px; white-space: nowrap; }
        .datatable th { background: rgba(241, 245, 249, 0.8); color: #475569; font-weight: 800; text-align: left; padding: 14px 18px; border-bottom: 1px solid #e2e8f0; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; }
        .datatable td { padding: 14px 18px; border-bottom: 1px solid #f1f5f9; color: #1e293b; font-weight: 500; }
        .datatable tr:hover td { background: rgba(255, 255, 255, 0.9); }
        ...
        .sidebar-link { transition: all 0.3s; position: relative; color: #94a3b8; font-weight: 500; }
        ...
        .tab-btn { padding: 12px 24px; background: transparent; border: none; border-bottom: 2px solid transparent; cursor: pointer; font-weight: 700; color: #64748b; transition: 0.3s; }
        ...
        .modal-content { background: #ffffff; margin: 8% auto; padding: 32px; width: 600px; max-width: 95%; border-radius: 24px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.3); border: 1px solid #e2e8f0; }
    </style>
</head>
...
<!-- 🎛️ Sidebar (Obsidian Glass) -->
<aside id="sidebar" class="sidebar-obsidian fixed top-0 left-0 z-50 w-64 h-screen overflow-y-auto transform -translate-x-full transition-transform duration-300 ease-in-out md:translate-x-0">
    <div class="p-6 flex items-center justify-between sticky top-0 bg-slate-900/90 backdrop-blur-xl z-10 border-b border-white/5">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-gradient-to-tr from-blue-600 to-cyan-400 rounded-xl flex items-center justify-center p-0.5 shadow-lg shadow-blue-500/30">
                <div class="w-full h-full bg-slate-900 rounded-[10px] flex items-center justify-center overflow-hidden p-1">
                    <img src="/logo.png" onerror="this.style.display='none'; this.parentElement.innerHTML='<span class=\\'text-white font-bold text-sm\\'>AGC</span>'" class="w-full h-full object-contain">
                </div>
            </div>
            <div> 
                <h1 class="text-white font-extrabold text-[17px] tracking-tight leading-none">AGC ERP</h1> 
                <p class="text-[10px] text-blue-400 uppercase tracking-widest font-bold mt-0.5">Enterprise</p> 
            </div>
        </div>
    ...
    <nav class="p-4 space-y-1 text-sm pb-20">
        <a href="/" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg"> <i class="fas fa-layer-group w-5 text-[15px]"></i> Dashboard </a>
        {% if session.get('role') == 'CUSTOMER' %}
        <a href="/booking" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg"> <i class="fas fa-plus-circle w-5 text-[15px]"></i> New Booking </a>
        <a href="/customer_bulk" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg"> <i class="fas fa-file-excel w-5 text-[15px]"></i> Bulk Upload </a>
        <a href="/pickup" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg"> <i class="fas fa-truck-pickup w-5 text-[15px]"></i> Request Pickup </a>
        <a href="/address_book" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg"> <i class="fas fa-address-book w-5 text-[15px]"></i> Address Book </a>
        <a href="/developer_api" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg"> <i class="fas fa-code w-5 text-[15px]"></i> API Integration </a>
        <a href="/shipments" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg"> <i class="fas fa-box-open w-5 text-[15px]"></i> My Shipments </a>
        <a href="/my_ledger" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg"> <i class="fas fa-file-invoice-dollar w-5 text-[15px]"></i> My Ledger </a>
        <a href="/wallet" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg"> <i class="fas fa-wallet w-5 text-[15px]"></i> Wallet </a>
        {% else %}
        <div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-6 mb-2 px-4">Master</div>
        <a href="/customers" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-building w-5 text-[15px]"></i> Franchisee </a>
        <a href="/cargo_master" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-handshake w-5 text-[15px]"></i> Cargo Party </a>
        <a href="/credit_party" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-credit-card w-5 text-[15px]"></i> Credit Party </a>
        ...
        <div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-6 mb-2 px-4">Operations</div>
        <a href="/booking" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-keyboard w-5 text-[15px]"></i> Booking </a>
        <a href="/inward" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-arrow-down w-5 text-[15px]"></i> Inward </a>
        <a href="/outward" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-arrow-up w-5 text-[15px]"></i> Outward </a>
        <a href="/manage_pickups" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-people-carry w-5 text-[15px]"></i> Pickups </a>
        <a href="/master_bag" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-shopping-bag w-5 text-[15px]"></i> Master Bag </a>
        <a href="/drs" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-clipboard-list w-5 text-[15px]"></i> D.R.S. </a>
        
        <div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-6 mb-2 px-4">Finance</div>
        <a href="/invoices" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-file-invoice w-5 text-[15px]"></i> Billing </a>
        <a href="/accounts" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-book w-5 text-[15px]"></i> Cash/Bank </a>
        <a href="/expenses" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-receipt w-5 text-[15px]"></i> Voucher </a>
        <a href="/party_ledger" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-balance-scale w-5 text-[15px]"></i> Ledger </a>
        <a href="/manage_wallets" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-wallet w-5 text-[15px]"></i> Wallets </a>
        
        <div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-6 mb-2 px-4">Tools</div>
        <a href="/reports" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-chart-line w-5 text-[15px]"></i> Reports </a>
        <a href="/shipments" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-list w-5 text-[15px]"></i> Status </a>
        <a href="/import_csv" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-cloud-upload-alt w-5 text-[15px]"></i> Import </a>
        <a href="/settings" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg"> <i class="fas fa-cog w-5 text-[15px]"></i> Settings </a>
        {% endif %}
    </nav>
</aside>
...
<div class="md:ml-64 min-h-screen flex flex-col w-full md:w-auto transition-all duration-300">
    ...
    <!-- 🔝 Header (Clean Pearl Glass) -->
    <header class="h-[76px] flex items-center justify-between px-8 sticky top-0 z-30 bg-white/60 backdrop-blur-xl border-b border-white shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
        <div class="flex items-center gap-4">
            <button onclick="toggleSidebar()" class="md:hidden text-slate-600 hover:text-blue-600 focus:outline-none">
        ...
        <div class="flex items-center gap-6">
            <form action="/track_doc" method="POST" target="_blank" class="hidden sm:flex items-center bg-white border border-slate-200 rounded-full px-4 py-2 shadow-sm focus-within:ring-2 focus-within:ring-blue-100 transition-all">
                <input type="hidden" name="_csrf_token" value="{{ csrf_token() }}">
                <i class="fas fa-search text-slate-400 mr-2 text-sm"></i>
                <input type="text" name="awb" placeholder="Search AWB..." class="bg-transparent outline-none text-sm w-48 font-semibold text-slate-700 placeholder-slate-400">
                <input type="hidden" name="doc_type" value="c_note">
            </form>
            
            <div class="flex items-center gap-4 pl-6 border-l border-slate-200">
                <div class="text-right hidden md:block"> 
                    <p class="text-sm font-extrabold text-slate-800 leading-tight">{{ session.get('full_name','Admin') }}</p> 
                    <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{{ session.get('role') }} • {{ session.get('branch','HQ') }}</p> 
                </div>
                <a href="/logout" class="w-10 h-10 bg-slate-100 text-slate-500 rounded-full flex items-center justify-center hover:bg-red-50 hover:text-red-500 transition-colors" title="Logout"> 
    ...
    <main class="p-6 md:p-8 flex-1 w-full max-w-full overflow-x-hidden">
        ...
        <div class="mb-6 space-y-3">
            ...
            <div class="p-4 rounded-2xl border flex items-center gap-3 font-semibold shadow-sm {{ 'bg-emerald-50 border-emerald-100 text-emerald-800' if category == 'success' else 'bg-rose-50 border-rose-100 text-rose-800' }}">
                <i class="fas {{ 'fa-check-circle text-emerald-500' if category == 'success' else 'fa-exclamation-circle text-rose-500' }} text-lg"></i>
                {{ message }}
            </div>
            {% endfor %}
        </div>
...
<script>
    ...
    $(document).ready(function(){
        $('.datatable').wrap('<div class="table-responsive-wrapper shadow-sm border border-slate-200 bg-white"></div>');
        if($('.datatable').length){
            $('.datatable').DataTable({ 
                "pageLength": 50, 
                "order": [],
            ...
            $('.dataTables_filter input').addClass('input-modern').css({'margin-left':'10px', 'width':'220px', 'display':'inline-block', 'padding':'8px 12px'});
        }
    });
</script>
</body>
...
def track():
    ...
    <head>
        ...
        <style>
            ...
            .search-box { background: rgba(255, 255, 255, 0.9); box-shadow: 0 8px 32px rgba(0,0,0,0.05), inset 0 2px 4px rgba(0,0,0,0.02); border: 2px solid transparent; transition: 0.3s; }
    ...
    <body class="antialiased min-h-screen flex flex-col">
        ...
        <main class="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 py-10">
            ...
            <div class="glass-panel p-6 border-l-4 border-l-red-500 mb-8 flex items-center gap-4">
                ...
                <div><h3 class="text-red-800 font-bold text-lg">Error processing request</h3><p class="text-red-600 font-medium">{{ error_msg }}</p></div>
            </div>
            {% elif awb and not shipment %}
            <div class="glass-panel p-10 mb-8 text-center">
                <div class="w-24 h-24 bg-white rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl shadow-amber-500/20 border-4 border-amber-50 text-amber-500 text-4xl"><i class="fas fa-box-open"></i></div>
            ...
            <div class="glass-panel p-8 mb-8 relative overflow-hidden">
                ...
                <div class="flex flex-col md:flex-row md:items-start justify-between border-b border-white/50 pb-6 mb-8 relative z-10">
                    <div class="flex items-center gap-6">
                        <div>
                    ...
                    <div class="mt-4 md:mt-0 text-left md:text-right">
                        ...
                        <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-black uppercase tracking-widest bg-white border border-white/60 shadow-[0_4px_15px_rgba(0,0,0,0.05)]
                            {% if status == 'DELIVERED' %}text-emerald-600{% elif status == 'CANCELLED' %}text-rose-600{% elif status == 'ON_DRS' %}text-blue-600{% elif status == 'UNDELIVERED' or status == 'RETURNED' %}text-amber-600{% else %}text-indigo-600{% endif %}">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white/40 p-6 rounded-2xl border border-white shadow-inner relative z-10 mt-12 sm:mt-0">
                    <div><p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Origin</p><p class="font-black text-slate-800 text-sm truncate" title="{{ shipment.origin_name }}">{{ shipment.origin_name }}</p></div>
                    <div><p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Destination</p><p class="font-black text-slate-800 text-sm truncate" title="{{ shipment.dest_name }}">{{ shipment.dest_name }}</p></div>
                    <div><p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Booked On</p><p class="font-black text-slate-800 text-sm">{{ shipment.booking_date }}</p></div>
                    <div><p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Weight / Pcs</p><p class="font-black text-slate-800 text-sm">{{ shipment.weight_kg }} KG <span class="text-blue-600 font-bold ml-1">({{ shipment.quantity }} Pcs)</span></p></div>
                </div>
            </div>
                ...
                <div class="relative border-l-2 border-blue-200/60 ml-4 md:ml-8 pb-4">
                    ...
                    <div class="mb-8 ml-8 md:ml-12 relative group">
                        ...
                        <div class="bg-white/80 backdrop-blur-sm border border-white rounded-2xl p-5 shadow-[0_4px_20px_-5px_rgba(0,0,0,0.05)] transition-all hover:shadow-[0_8px_30px_-5px_rgba(0,0,0,0.1)] hover:-translate-y-1">
                            ...
                            <p class="text-slate-700 font-bold flex items-center gap-2 text-sm"><i class="fas fa-map-marker-alt text-slate-400 w-4 text-center"></i> {{ e.location }}</p>
                            {% if e.remarks %}
                            <div class="mt-3 bg-blue-50/50 p-3 rounded-xl border border-blue-100/50"><p class="text-xs text-slate-600 font-bold flex items-start gap-2"><i class="fas fa-info-circle text-blue-500 mt-0.5"></i> <span>{{ e.remarks }}</span></p></div>
                            {% endif %}
                        </div>
                    </div>
                    {% endfor %}
...
def stationery():
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="card lg:col-span-2">
            ...
            <div class="table-responsive">
            <table class="datatable">
                <thead><tr><th>Date</th><th>Party Name</th><th>Prefix</th><th>Range</th><th>Qty</th><th>Actions</th></tr></thead>
                <tbody>
                {% for b in batches %}
                <tr>
                    ...
                    <td>{{ b.start_no }} to {{ b.end_no }}</td>
                    <td><span class="px-2 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-bold">{{ b.qty }}</span></td>
                    <td style="white-space:nowrap;">
                        <a href="/print/stationery_barcodes/{{ b.id }}" target="_blank" class="btn-warning" style="padding:4px 8px; font-size:11px;"><i class="fas fa-barcode"></i> Sticker</a>
                        <a href="/print/stationery_cnotes/{{ b.id }}" target="_blank" class="btn-success" style="padding:4px 8px; font-size:11px;"><i class="fas fa-print"></i> C-Note</a>
...
# ==========================================
# 🖨️ WEB ERP: PRINT LABEL & RECEIPT (LOGO + QR + STATIONS)
# ==========================================
import io, os
...
def draw_qr_web(cv, text, x, y, size=60):
    try:
        img = qrcode.make(text)
        buf = io.BytesIO()
        img.save(buf, format="PNG")
        buf.seek(0)
        cv.drawImage(ImageReader(buf), x, y, width=size, height=size)
    except Exception as e:
        print("QR Error:", e)

def draw_logo_web(cv, x, y, width, height):
    # Desktop ka path ya Web ka static path
    logo_paths = ['logo.png', 'static/logo.png']
    for lp in logo_paths:
        if os.path.exists(lp):
            try:
                cv.drawImage(ImageReader(lp), x, y, width=width, height=height, preserveAspectRatio=True, mask="auto")
                return True
            except: pass
    return False
...
def print_label(awb):
    ...
    try:
        ...
        shipper_info += f" | GSTIN: {cust.get('gstin') or '-'}"
...
def print_receipt(awb):
    ...
    try:
        ...
        buf = io.BytesIO()
        w, h = 595, 421 # A5 Size for Receipt
        cv = canvas.Canvas(buf, pagesize=(w, h))
        ...
        cv.setFillColorRGB(*hex_rgb("#23272F")); cv.setFont("Helvetica", 7.5)
...
def address_book():
    ...
    # 🚀 Auto-Heal: Create Table & Add Missing Pincode Column (Crash-Proof)
    with conn.cursor() as c:
        c.execute("""CREATE TABLE IF NOT EXISTS address_book (
            ...
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP)""")
        try: c.execute("ALTER TABLE address_book ADD COLUMN pincode VARCHAR(20)")
        except: pass
    ...
    if request.method == 'POST':
        ...
        with conn.cursor() as c:
            if action == 'add':
                ...
                c.execute("""INSERT INTO address_book (customer_id, name, phone, address, station, state_code, pincode) 
                             VALUES (%s,%s,%s,%s,%s,%s,%s)""",
                          (cid, request.form.get('name'), request.form.get('phone'), 
                           request.form.get('address'), request.form.get('station').upper(), 
                           request.form.get('state_code'), request.form.get('pincode')))
                flash("✅ Address saved to your Address Book!", "success")
                
            elif action == 'edit':
                c.execute("INSERT IGNORE INTO stations(name) VALUES(%s)", (request.form.get('station').upper(),))
                c.execute("""UPDATE address_book SET name=%s, phone=%s, station=%s, state_code=%s, pincode=%s, address=%s 
                             WHERE id=%s AND customer_id=%s""",
                          (request.form.get('name'), request.form.get('phone'), 
                           request.form.get('station').upper(), request.form.get('state_code'), 
                           request.form.get('pincode'), request.form.get('address'), 
                           request.form.get('edit_id'), cid))
    ...
    html = """
    <!-- 🌟 TOP SECTION: MODERN ADDRESS FORM -->
    <div class="card mb-6" style="border-top:4px solid #10b981;">
        <h3 class="text-xl font-black text-slate-800 mb-6 flex items-center gap-3">
            <div class="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center"><i class="fas fa-address-card"></i></div>
            Add New Receiver
        </h3>
        ...
        <form method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div><label class="label-modern">Receiver Name *</label><input type="text" name="name" placeholder="e.g. Rahul Sharma" required class="input-modern font-bold text-slate-800"></div>
                <div><label class="label-modern">Phone Number</label><input type="text" name="phone" placeholder="10-digit mobile number" class="input-modern"></div>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    <label class="label-modern">Dest Station / City *</label>
                    <input type="text" name="station" list="st_list" class="input-modern uppercase font-bold text-blue-600" placeholder="Type city name..." required>
                    <datalist id="st_list">{% for s in stations %}<option value="{{ s.name }}">{% endfor %}</datalist>
                </div>
                <div><label class="label-modern">Pincode</label><input type="text" name="pincode" class="input-modern" placeholder="6-digit pincode" maxlength="6"></div>
                <div><label class="label-modern">State Code</label><input type="text" name="state_code" class="input-modern uppercase" placeholder="e.g. RJ" maxlength="2"></div>
            </div>
            
            <div><label class="label-modern">Full Address</label><textarea name="address" rows="2" placeholder="House No, Street, Landmark..." class="input-modern"></textarea></div>
            
            <div class="flex justify-end pt-2">
                <button type="submit" class="btn-success px-10 py-3 shadow-lg"><i class="fas fa-save mr-2"></i> Save to Address Book</button>
            </div>
        </form>
    </div>

    <!-- 🌟 BOTTOM SECTION: FULL WIDTH ADDRESS LIST -->
    <div class="card">
        <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
            <h3 class="text-xl font-black text-slate-800 flex items-center gap-3">
                <div class="w-10 h-10 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center"><i class="fas fa-list-ul"></i></div>
                Saved Addresses
            </h3>
            <span class="px-4 py-1.5 bg-slate-100 text-slate-600 rounded-full text-sm font-bold shadow-inner">{{ addrs|length }} Records</span>
        </div>
        
        <div class="table-responsive">
            <table class="datatable w-full">
                <thead><tr><th>Name / Phone</th><th>Destination Info</th><th>Detailed Address</th><th style="width:120px; text-align:center;">Action</th></tr></thead>
                <tbody>
                {% for a in addrs %}
                <tr class="hover:bg-slate-50 transition-colors">
                    <td>
                        <span class="font-bold text-blue-600 text-[15px]">{{ a.name }}</span><br>
                        <span class="text-xs text-slate-500 font-bold"><i class="fas fa-phone-alt text-slate-400 mr-1"></i> {{ a.phone or 'N/A' }}</span>
                    </td>
                    <td>
                        <span class="font-bold text-slate-800 text-[14px]">{{ a.station }}</span><br>
                        <span class="text-xs text-slate-500 font-bold">State: <span class="text-amber-600">{{ a.state_code or '-' }}</span> &nbsp;|&nbsp; Pin: <span class="text-emerald-600">{{ a.pincode or '-' }}</span></span>
                    </td>
                    <td class="text-sm text-slate-600 max-w-xs truncate" title="{{ a.address }}">{{ a.address or '-' }}</td>
                    <td style="text-align:center;">
                        <!-- Edit Button -->
                        <button type="button" onclick="openAddressEdit('{{ a.id }}', '{{ a.name }}', '{{ a.phone }}', '{{ a.station }}', '{{ a.pincode }}', '{{ a.state_code }}', '{{ a.address | replace('\n', ' ') | replace('\r', '') }}')" class="btn-primary" style="padding:6px 12px; font-size:12px; margin-right:4px;" title="Edit Address"><i class="fas fa-edit"></i></button>
                        
                        <!-- Delete Button -->
                        <form method="POST" style="display:inline;" onsubmit="return confirm('Delete this address permanently?');">
                            <input type="hidden" name="action" value="delete"><input type="hidden" name="del_id" value="{{ a.id }}">
                            <button type="submit" class="btn-danger" style="padding:6px 12px; font-size:12px;" title="Delete Address"><i class="fas fa-trash"></i></button>
                        </form>
                    </td>
                </tr>
                {% endfor %}
    ...
    <!-- 🌟 EDIT ADDRESS MODAL (Properly formatted) -->
    <div id="editAddrModal" class="modal">
        <div class="modal-content" style="max-width: 600px;">
            <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-3">
                <h3 class="text-xl font-black text-slate-800">✏️ Edit Address</h3>
                <button onclick="document.getElementById('editAddrModal').style.display='none'" class="text-slate-400 hover:text-red-500"><i class="fas fa-times text-xl"></i></button>
            </div>
            ...
            <form method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label class="label-modern">Receiver Name *</label><input type="text" name="name" id="ea_name" required class="input-modern font-bold text-slate-800"></div>
                    <div><label class="label-modern">Phone</label><input type="text" name="phone" id="ea_phone" class="input-modern"></div>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div><label class="label-modern">Dest Station *</label><input type="text" name="station" id="ea_station" list="st_list" class="input-modern uppercase font-bold text-blue-600" required></div>
                    <div><label class="label-modern">Pincode</label><input type="text" name="pincode" id="ea_pincode" class="input-modern" maxlength="6"></div>
                    <div><label class="label-modern">State Code</label><input type="text" name="state_code" id="ea_state" class="input-modern uppercase" maxlength="2"></div>
                </div>
                
                <div><label class="label-modern">Full Address</label><textarea name="address" id="ea_addr" rows="2" class="input-modern"></textarea></div>
                
                <div class="flex gap-3 pt-4 border-t border-slate-100">
                    <button type="button" class="btn-danger flex-1 py-3" onclick="document.getElementById('editAddrModal').style.display='none'">Cancel</button>
                    <button type="submit" class="btn-success flex-1 py-3"><i class="fas fa-save mr-2"></i> Update Details</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
    function openAddressEdit(id, name, phone, station, pincode, state, addr) {
        ...
        document.getElementById('ea_pincode').value = pincode === 'None' ? '' : pincode;
        document.getElementById('ea_state').value = state === 'None' ? '' : state;
        document.getElementById('ea_addr').value = addr === 'None' ? '' : addr;
