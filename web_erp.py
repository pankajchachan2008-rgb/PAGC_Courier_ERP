    ...
    try:
        ...
        with conn.cursor() as c:
            ...
            defs = {
                ...
                "company_website": "https://pagcerp.cgsmart.in", "company_email": "PANKAJNOHAR@YAHOO.CO.IN",
...
# ==========================================
# 🆕 4. PUBLIC TRACKING PAGE (NO LOGIN REQUIRED)
# ==========================================
...
def users():
    ...
    {% for u in u_list %}
    <tr>
        <td><strong>{{ u.username }}</strong></td><td>{{ u.full_name }}</td><td><span class="badge">{{ u.role }}</span></td><td>{{ u.branch_name or 'HQ' }}</td>
...
def booking():
    ...
    function fetchRate() {
        ...
        if(cid) { let opt = document.getElementById('cid').options[document.getElementById('cid').selectedIndex]; document.getElementById('ost').value = opt.getAttribute('data-state'); }
            ...
            <tr>
                ...
                <td>
                    ...
                    {% if ph %}
                        ...
                        <a href="https://wa.me/91{{ clean_ph[-10:] }}?text=Track%20AGC%20Parcel:%20https://pagcerp.cgsmart.in/track?awb={{ r.awb_no }}" target="_blank" class="btn" style="background:#12B76A; padding:4px; font-size:11px;"><i class="fab fa-whatsapp"></i></a>
                    {% endif %}
...
# ==========================================
# 🖨️ 7. PDF GENERATION ENGINE (ReportLab)
# ==========================================
...
def print_label_pdf(awb):
    ...
    cv.setLineWidth(1)
    cv.setFont("Helvetica-Bold", 14); cv.drawString(4*mm, 137*mm, get_setting("company_name", "AKASH GANGA COURIER"))
    cv.setFont("Helvetica", 6); cv.drawString(4*mm, 130*mm, "ISO 9001:2008 Certified Company")
...
# ==========================================
# 🚀 8. OPERATIONS: OUTWARD, INWARD, DRS, MASTER BAG
# (Condensed for brevity, but fully functional logic from your offline code)
# ==========================================
...
def outward():
    ...
    if request.method == 'POST' and request.form.get('action') == 'save_entry':
        ...
        if awb:
            with conn.cursor() as c:
                ...
                c.execute("INSERT INTO outward_register(entry_date, awb_no, origin_station, out_station, destination, weight, pcs, network, network_awb, bag_no, info, finalized) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 0)", (o_date, awb, session['branch'], o_station, final_dest, final_wt, 1, 'SELF', '', '', info))
                conn.commit()
    ...
    with conn.cursor() as c:
        ...
        c.execute("SELECT id, manifest_no, created_at, from_location, to_location, vehicle_no FROM manifests WHERE manifest_type='OUTWARD' ORDER BY id DESC LIMIT 10"); mans = c.fetchall()
    conn.close()
    
    html = """
    <div class="card" id="content-new">
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:15px;">
            <label>Date</label><input type="date" id="ui_date" value="{{ f_date }}" onchange="window.location.href=`/outward?date=${this.value}&station=${document.getElementById('ui_station').value}`" style="width:130px;">
            <label>Station</label><input list="stlist" id="ui_station" value="{{ f_station }}" onchange="window.location.href=`/outward?date=${document.getElementById('ui_date').value}&station=${this.value}`" style="width:150px;"><datalist id="stlist">{% for s in stations %}<option value="{{ s }}">{% endfor %}</datalist>
        </div>
        <form method="POST" id="entryForm" style="display:flex; gap:10px; background:#13294B; padding:10px; border-radius:6px; margin-bottom:10px;">
            <input type="hidden" name="action" value="save_entry"><input type="hidden" name="out_date" value="{{ f_date }}"><input type="hidden" name="out_station" value="{{ f_station }}">
            <input type="text" name="awb" placeholder="AWB" required autofocus style="flex:1;">
            <input type="text" name="dest" list="stlist" placeholder="Dest" style="flex:1;">
            <input type="number" step="0.01" name="weight" placeholder="Wt" style="width:70px;">
            <input type="text" name="info" placeholder="Info" style="flex:1;">
        ...
        <div style="height:250px; overflow-y:auto; border:1px solid #E1E6EE; border-radius:4px;"><table style="margin:0;">
            <tr style="position:sticky; top:0; background:#F5F7FA; z-index:1;"><th>AWB</th><th>Dest</th><th>Wt</th><th>Info</th></tr>
            {% for p in pending_list %}<tr><td style="color:#0E8A6D; font-weight:bold;">{{ p.awb_no }}</td><td>{{ p.destination }}</td><td>{{ p.weight }}</td><td>{{ p.info }}</td></tr>{% endfor %}
        </table></div>
        <form method="POST" id="finalizeForm" style="display:flex; gap:10px; margin-top:15px;">
            <input type="hidden" name="action" value="finalize"><input type="hidden" name="out_date" value="{{ f_date }}"><input type="hidden" name="out_station" value="{{ f_station }}">
            ...
            <button type="submit" class="btn btn-gold" style="flex:1;">🔒 FINALIZE MANIFEST</button>
        </form>
    </div>
    <div class="card" style="margin-top:20px;">
        <h3>Recent Manifests</h3>
        <table><tr><th>Manifest No</th><th>Date</th><th>Route</th><th>Vehicle</th></tr>
        {% for m in mans %}<tr><td><strong>{{ m.manifest_no }}</strong></td><td>{{ m.created_at }}</td><td>{{ m.from_location }} &rarr; {{ m.to_location }}</td><td>{{ m.vehicle_no }}</td></tr>{% endfor %}</table>
    </div>
    """
...
def master_bag():
    ...
    conn.close()
    html = """<div class="grid-2"><div class="card"><h3 style="color:#0E8A6D; margin-top:0;">🎒 Create Master Bag</h3><form method="POST"><label>Dest Hub</label><input name="dest_hub" list="stations" required style="margin-bottom:15px; text-transform:uppercase;"><datalist id="stations">{% for s in stations %}<option value="{{ s.name }}">{% endfor %}</datalist><textarea name="awbs" rows="6" required placeholder="AWBs (comma or newline separated)" style="font-family:monospace; font-size:14px; margin-top:5px;"></textarea><button type="submit" class="btn btn-blue" style="margin-top:10px; width:100%; font-size:15px;">🔒 SEAL MASTER BAG</button></form></div><div class="card"><h3>Recent Bags</h3><table><tr><th>Bag No</th><th>Dest</th><th>Items</th></tr>{% for b in bags %}<tr><td style="color:#0E8A6D;"><strong>{{ b.bag_no }}</strong></td><td>{{ b.destination }}</td><td>{{ b.items }}</td></tr>{% endfor %}</table></div></div>"""
    return render_page("MASTER BAG", render_template_string(html, stations=stations, bags=bags))

# ==========================================
# 💰 9. ACCOUNTS, EXPENSES & REPORTS
# ==========================================
...
def reports():
    ...
    with conn.cursor() as c:
        ...
        if session.get('role') != 'ADMIN': q_b += " AND origin_name=%s"; p1.append(session['branch'])
    ...
    <div class="card"><h3 style="color:#D64550;">🔴 Market Outstanding</h3><table><tr><th>Customer</th><th>Due Amount</th></tr>{% for o in out %}<tr><td><strong>{{ o.name }}</strong></td><td style="color:#D64550; font-weight:bold;">₹{{ o.bal }}</td></tr>{% endfor %}</table></div>
    """
...
if __name__ == '__main__':
    # Production ready: Use gunicorn in real deployment
