...
<head>
    ...
    <style>
        ...
        .sidebar { width: 250px; background: #0B1F3A; color: white; position: fixed; height: 100%; overflow-y: auto; box-shadow: 2px 0 10px rgba(0,0,0,0.2); }
        .logo { padding: 20px; font-size: 22px; font-weight: 900; color: #C9A24B; border-bottom: 1px solid #13294B; text-align: center; }
        .menu a { display: block; padding: 12px 25px; color: #B8C4D6; text-decoration: none; font-weight: 600; font-size:13px; transition: 0.2s; }
        ...
        label { font-weight: 600; color: #7A8699; margin-right: 5px; font-size: 12px; }
...
<body>
    <div class="sidebar">
        ...
        <div class="menu">
            ...
            <a href="/rates"><i class="fas fa-tags"></i> Rate Cards</a>
            ...
            <a href="/reports"><i class="fas fa-chart-bar"></i> Master Reports</a>
            
            {% if session.get('role') == 'ADMIN' %}
                <div class="menu-header">⚙️ ADMINISTRATION</div>
                <a href="/stationery"><i class="fas fa-barcode"></i> Stationery AWB</a>
                <a href="/users"><i class="fas fa-user-shield"></i> Users & Branch</a>
                <a href="/settings"><i class="fas fa-cogs"></i> System Settings</a>
                <a href="/import_csv"><i class="fas fa-file-import"></i> Excel Import</a>
            {% endif %}
            <a href="/logout" style="color:#D64550; margin-top:10px;"><i class="fas fa-power-off"></i> Logout</a>
        </div>
    </div>
    <div class="main-content">
        <div class="header">
            ...
            <div style="display:flex; gap:15px; align-items:center;">
                <input type="text" placeholder="🔍 Search AWB..." style="width:200px; border-radius:20px;">
...
def dashboard():
    ...
    <div class="card"><h3>📦 Recent Bookings ({session['branch']})</h3>
        ...
        {''.join(f"<tr><td><strong>{r['awb_no']}</strong></td><td>{r['booking_date']}</td><td>{r['dest_name']}</td><td>₹{r['total_amount']}</td><td><span class='badge b-book'>{r['status']}</span></td></tr>" for r in latest)}</table>
    </div>
    """
    return render_page("Executive Dashboard", html)

# ==========================================
# ⚙️ 4. SETTINGS, RATES & STATIONERY (NEW ADMIN FEATURES)
# ==========================================
...
# ==========================================
# 📦 5. BOOKING & SHIPMENTS (WITH DYNAMIC RATES)
# ==========================================
...
def booking():
    ...
    if request.method == 'POST':
        d = request.form
        # Fallbacks to ensure float conversion
    ...
    if request.method == 'POST':
        ...
        with conn.cursor() as c:
            for row in reader:
                ...
                if c.fetchone(): continue # Skip dup
